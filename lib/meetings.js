...
export const MEETINGS = [
  ...
  // 広報チームの週次キックオフ。広報は経営企画部配下の単一チームのため
  // specific-team で teamName='広報' を指定する (= 営業定例 sales と同型)。
  // key='weekly-pr' は organization_meetings に既存登録された行と一致させる
  // (DB行は selection 画面の表示用、ファシリ画面は静的 MEETINGS の flow:'ka' を採用)。
  { key: 'weekly-pr',            title: '週次キックオフ（広報）',             schedule: '月曜',     icon: '📢', color: '#ec4899',
    weeklyMTG: { scope: 'specific-team', teamName: '広報', flow: 'ka',
      levelName: '広報', viewMode: 'ka' } },
  ...
  // 経営企画会議: 経営企画部 配下のチーム (経営企画チーム / 事務局 / 等) の KR を順に確認
  { key: 'planning',             title: '経営企画会議',                      schedule: '木曜',     icon: '📋', color: '#ffd166',
    weeklyMTG: { scope: 'teams-of', parentLevelName: '経営企画部', flow: 'kr',
      levelName: '経営企画部', viewMode: 'kr' } },
  // 役員会議: 全事業部の合同 KR レビュー (旧「経営企画会議」の中身を踏襲)
  { key: 'board',                title: '役員会議',                          schedule: '木曜',     icon: '🏛️', color: '#5856d6',
    weeklyMTG: { scope: 'all-departments', flow: 'kr',
      levelName: null, viewMode: 'kr' } },

  // ── プログラム別定例 ───────────────────────────────────────
  // 機能別組織を横断するプログラム単位の会議。スコープは全階層 (全社 / 部署 /
  // チームすべて) — タグがどのレベルの obj に付いていても拾えるように。
  // 選択された program_tag に紐付く KR / KA だけが対象になる。
  // requiresProgram=true: 開始前にプログラムタグを選択しないと進めない。
  { key: 'program-regular',      title: 'プログラム別定例',                  schedule: '任意',     icon: '🏷', color: '#6B96C7',
    weeklyMTG: { scope: 'all-levels', flow: 'ka', requiresProgram: true,
      levelName: null, viewMode: 'ka' } },
]
...
export const MEETING_URLS = {
  ...
  // 経営企画会議は新仕様 (経営企画部チーム順送り)。Notion DB は当面 PLANNING_URL を流用
  'planning':           process.env.NEXT_PUBLIC_PLANNING_URL,
  // 役員会議: 旧 PLANNING の DB を踏襲。BOARD_URL があればそちらを優先
  'board':              process.env.NEXT_PUBLIC_BOARD_URL || process.env.NEXT_PUBLIC_PLANNING_URL,
  // プログラム別定例: 専用 Notion DB が設定されていなければ無効 (Notion 連携機能のみオフ)
  'program-regular':    process.env.NEXT_PUBLIC_PROGRAM_REGULAR_URL,
}
...
export function getMeetingDbId(key) {
  const map = {
    ...
    'board':              process.env.NOTION_BOARD_DB_ID || process.env.NOTION_PLANNING_DB_ID,
    'program-regular':    process.env.NOTION_PROGRAM_REGULAR_DB_ID,
  }
  return map[key]
}
