...
function rowToMeeting(row) {
  // weeklyMTG (旧 target_filter) を組み立てる。
  //   - target_filter に中身がある: それを使い、flow が無ければ modules から推定して補完
  //     (例: program-regular は scope/requiresProgram だけで flow が無い → KA重点に推定)
  //   - target_filter が空 (= {} or null): weeklyMTG=null を返す。
  //     → WeeklyMTGPage.lookupMeeting が静的 MEETINGS の weeklyMTG (scope/flow) で補完する。
  //     (例: 広報週次キックオフ。DB 行に target_filter が未設定でも静的定義で開ける)
  const tf = row.key !== 'morning' ? row.target_filter : null
  const tfHasContent = tf && typeof tf === 'object' && Object.keys(tf).length > 0
  const wkly = tfHasContent
    ? (tf.flow ? tf : { ...tf, flow: inferFlowFromModules(row.modules) || tf.flow })
    : null
  return {
    id: row.id,
    key: row.key,
...
export function useOrganizationMeetings() {
  ...
  // 会議の追加/編集/削除後に再取得するためのトリガ。
  // MeetingEditModal などが window イベント 'org-meetings-updated' を発火する。
  const [reloadTick, setReloadTick] = useState(0)
  useEffect(() => {
    const onUpdated = () => setReloadTick(t => t + 1)
    window.addEventListener('org-meetings-updated', onUpdated)
    return () => window.removeEventListener('org-meetings-updated', onUpdated)
  }, [])
  ...
  useEffect(() => {
    ...
    return () => { alive = false }
  }, [currentOrg?.id, currentOrg?.slug, orgLoading, reloadTick])
