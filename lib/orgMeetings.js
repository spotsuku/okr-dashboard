...
// target_filter が設定されていればそれを weeklyMTG として使用、未設定なら null。
// (null の場合は WeeklyMTGPage の lookupMeeting で静的 MEETINGS の weeklyMTG が
//  フォールバックとして使われる仕組み。これで既存固定会議の flow/scope が壊れない。)
function rowToMeeting(row) {
  const tfHasContent = row.target_filter && typeof row.target_filter === 'object' && Object.keys(row.target_filter).length > 0
  return {
    id: row.id,
    key: row.key,
    title: row.title,
    ...
    weeklyMTG: (row.key !== 'morning' && tfHasContent) ? row.target_filter : null,
  }
}
