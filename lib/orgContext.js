...
const OrgContext = createContext({
  ...
  switchOrg: () => {},
  reload: () => {},
  // 管理者が「メンバー目線」で UI を確認するためのプレビュー切替 (UI のみ。RLS には影響しない)
  viewAsMember: false,
  setViewAsMember: () => {},
})
...
export function OrgProvider({ user, initialSlug, children }) {
  ...
  // 管理者が「メンバー目線」で表示を確認するためのUIプレビュー切替
  ...
  const switchOrg = useCallback((slug) => {
    ...
    if (!next) return
    setCurrentOrg(next)
    setViewAsMember(false)  // 組織を切り替えたらプレビューは解除
    if (typeof window !== 'undefined') localStorage.setItem('current_org_slug', slug)
  }, [orgs])

  return (
    <OrgContext.Provider value={{ currentOrg, loading, error, orgs, switchOrg, reload, viewAsMember, setViewAsMember }}>
      {children}
    </OrgContext.Provider>
  )
}
