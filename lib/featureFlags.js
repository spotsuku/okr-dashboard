...
// 全組織で初期から使えるよう、既定で ON にするモジュール。
// (Google 連携 = メール/カレンダー/ドライブ、MyCOO は基本機能。連携トークンは
//  organization_id で分離済みのため、タブを全組織で出しても他組織のデータは見えない)
// organizations.enabled_modules に明示的な値があればそちらを尊重する。
export const DEFAULT_ENABLED_MODULES = {
  google_integration: true,
  coo_knowledge: true,
}

function withDefaults(mods) {
  return { ...DEFAULT_ENABLED_MODULES, ...(mods || {}) }
}
...
export function useFeatureFlag(moduleKey) {
  ...
  return !!withDefaults(currentOrg?.enabled_modules)[moduleKey]
}

export function useFeatureFlags() {
  const { currentOrg } = useCurrentOrg()
  return withDefaults(currentOrg?.enabled_modules)
}
