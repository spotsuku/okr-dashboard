...
//
// 任意 env:
//   INVITE_BASE_URL     : サインインURLのベース (デフォルト: https://aiworkspace.jp)
//
// 設定が無い場合は { skipped: true, reason } を返し、API ルートは
// メンバー作成自体は成功させたまま「メール送信のみスキップ」を返す方針。

const DEFAULT_BASE_URL = 'https://aiworkspace.jp'
...
export async function sendInviteEmail({
  ...
  const subject = `[${orgLabel}] AI WorkSpaceへの招待`

  const greet = toName ? `${toName} さん` : `${toEmail} さん`
  const text = [
    `${greet}`,
    ``,
    `${inviterEmail} さんから「${orgLabel}」のAI WorkSpaceに招待されました。`,
    `あなたのロール: ${roleLabel}`,
    ``,
...
<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif; color: #1a1a1a; max-width: 560px; margin: 0 auto; padding: 24px;">
  <h2 style="margin: 0 0 18px; font-size: 18px; font-weight: 700;">
    「${escapeHtml(orgLabel)}」のAI WorkSpaceへの招待
  </h2>
  <p style="margin: 0 0 14px; font-size: 14px; line-height: 1.7;">
    ${escapeHtml(greet)}
  </p>
  <p style="margin: 0 0 14px; font-size: 14px; line-height: 1.7;">
    <strong>${escapeHtml(inviterEmail)}</strong> さんから「<strong>${escapeHtml(orgLabel)}</strong>」のAI WorkSpaceに招待されました。<br/>
