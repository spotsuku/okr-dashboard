...
import { SHADOWS, RADIUS, SPACING, TYPO, TRANSITION, GLASS, BRAND_GRADIENT } from './themeTokens'

// ─── カード (Glass テーマ: 半透明フロスト) ───────────────────────────────
// 旧: accent カラーで縦カラーチントグラデ + 色付き左ボーダー
// 新: 半透明白 + backdrop-filter + glassInset 影 (= ガラスの艶)
//     accent は無視 (= 色は中身で示す。Glass テーマでは罫線・余白で階層を作る)
export function cardStyle({ T, accent, padding = SPACING.lg, raised = true }) {
  return {
    ...
    boxShadow: raised ? `${SHADOWS.xs}, ${SHADOWS.glassInset}` : SHADOWS.glassInset,
...
// ブランドCTA: BRAND_GRADIENT.cta (青→シアン)。"ここぞ" の主要CTAのみ
// (終業する / AI返信 / KR記入 / 年度トグルactive 等)。面積は画面の10〜15%まで。
export function btnBrand({ size = 'md' }) {
  return {
    border: 'none', cursor: 'pointer', fontFamily: 'inherit',
    background: BRAND_GRADIENT.cta,
    color: '#FFFFFF', fontWeight: 800,
    boxShadow: '0 2px 8px rgba(37,99,235,.28), 0 1px 2px rgba(0,0,0,0.06)',
    transition: TRANSITION.fast,
    ...btnSizes[size],
  }
}
