...
export async function POST(request) {
  try {
    ...
    // 組織名・紹介文:
    //   1. client から context.organizationName を渡されたらそれを優先 (SaaS化後の新テナント)
    //   2. なければ環境フォールバック (デモ=Humano Robotics / 本番=NEO福岡)
    // 既存 neo-fukuoka ユーザーは client から渡さなくても従来の prompt が出る (後方互換)
    const explicitOrgName = (context?.organizationName || '').trim()
    const explicitOrgDesc = (context?.organizationDescription || '').trim()
    const orgName = explicitOrgName || (isDemoMode ? 'Humano Robotics' : 'NEO福岡')

    let orgIntro
    if (orgName === 'Humano Robotics') {
      orgIntro = `【Humano Robotics Inc. について】
...
現在のフェーズ: シリーズB 30億円調達 / 量産フェーズ移行 / 海外展示 (CES2026/27) 準備中`
    } else if (orgName === 'NEO福岡' || orgName === 'NEO 福岡') {
      orgIntro = `【NEO福岡について】
...
- 評議会・アカデミア・研修などのチームが各事業部配下にあります`
      ...
      // 新テナント (SaaS化): client から渡された組織名 + 説明文ベースの汎用 prompt
      orgIntro = `【${orgName} について】
${explicitOrgDesc || `${orgName} は OKR と週次レビューで組織運営を進めています。メンバー一人ひとりが目標に対して自走できるよう支援することが目的です。`}`
    }

    // 現在日時 (JST) と今週の範囲を確定値で算出し、プロンプトに明示する。
    // これが無いとモデルが「今日」「今週」「今週末」の日付を誤って推論する。
    ...
    const dateBlock = `【現在日時 (日本時間 JST・確定値)】
- 今日: ${fmtJ(mkJ(0))} ${hh}:${mm}
- 今週 (週は月曜始まり・日曜終わり): ${fmtJ(mkJ(toMonday))} 〜 ${fmtJ(mkJ(toMonday + 6))}
- 今週末 (= 土曜・日曜): ${fmtJ(mkJ(toMonday + 5))}〜${fmtJ(mkJ(toMonday + 6))}
- 平日 = 月〜金 / 週末 = 土・日
※「今日」「明日」「今週」「今週末」「来週」などの日付・曜日は必ずこの確定値に一致させ、自分で推論しないこと。
`
