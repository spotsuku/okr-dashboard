...
function calcKRProgress(kr) {
  ...
  const raw = kr.lower_is_better
    ? Math.max(0, ((kr.target * 2 - kr.current_value) / kr.target) * 100)
    : (kr.current_value / kr.target) * 100
...
function ObjForm({ initial, onSave, onClose, levels, activeLevelId, activePeriod }) {
  ...
  const [krs, setKRs] = useState(
    initial?.key_results?.length
      ? initial.key_results.map(k => ({ ...k, target: String(k.target), current_value: String(k.current_value) }))
      : [{ _tmpId: Date.now(), title: '', target: '', current_value: '', unit: '', lower_is_better: false }]
  )
  const [saving, setSaving] = useState(false)

  const addKR = () => setKRs(p => [...p, { _tmpId: Date.now(), title: '', target: '', current_value: '', unit: '', lower_is_better: false }])
  ...
  const save = async () => {
    ...
    await onSave({
      ...
      krs: krs.map(k => ({ ...k, target: parseFloat(k.target) || 0, current_value: parseFloat(k.current_value) || 0 })),
    })
    setSaving(false)
    onClose()
  }
        ...
        return (
          <div key={key} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, padding: '12px 14px', marginBottom: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <div style={{ flex: 1 }}><FInput value={String(kr.current_value)} onChange={v => updateKR(key, 'current_value', v)} placeholder="現在値" type="number" /></div>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 5, gap: 8 }}>
                      ...
                      <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 10, color: '#505878', flexShrink: 0 }}>
                        {kr.current_value}{kr.unit} / {kr.target}{kr.unit}
...
export default function Dashboard({ user, onSignOut }) {
  ...
  // Load objectives + key_results for current level/period
  const loadObjectives = useCallback(async (levelId, period) => {
    if (!levelId) return
  ...
  useEffect(() => { loadObjectives(activeLevelId, activePeriod) }, [activeLevelId, activePeriod, loadObjectives])
  ...
  const handleSave = async ({ obj, krs }) => {
    if (obj.id) {
      ...
      if (krs.length) await supabase.from('key_results').insert(krs.map(k => ({ title: k.title, target: k.target, current_value: k.current_value, unit: k.unit, lower_is_better: k.lower_is_better, objective_id: obj.id })))
    } else {
      // Insert new
      const { data } = await supabase.from('objectives').insert([{ title: obj.title, owner: obj.owner, level_id: obj.level_id, period: obj.period }]).select().single()
      if (data && krs.length) await supabase.from('key_results').insert(krs.map(k => ({ title: k.title, target: k.target, current_value: k.current_value, unit: k.unit, lower_is_better: k.lower_is_better, objective_id: data.id })))
    }
    loadObjectives(activeLevelId, activePeriod)
  }
  ...
  const getChildren = id => levels.filter(l => l.parent_id === id)
    ...
    <div style={{ minHeight: '100vh', background: '#090d18', color: '#e8eaf0', fontFamily: "'Noto Sans JP','Hiragino Sans',sans-serif", display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#4d9fff', boxShadow: '0 0 10px #4d9fff' }} />
          <div>
            <div style={{ fontSize: 10, color: '#4d9fff', letterSpacing: '0.18em', textTransform: 'uppercase', marginBottom: 1 }}>OKR Management</div>
            <div style={{ fontSize: 16, fontWeight: 700, letterSpacing: '-0.02em' }}>OKR ダッシュボード</div>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 3, background: 'rgba(255,255,255,0.04)', padding: 3, borderRadius: 9, border: '1px solid rgba(255,255,255,0.06)' }}>
          {periods.map(p => (
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          <span style={{ fontSize: 11, color: '#505878' }}>{user.email}</span>
          <Btn small variant="ghost" onClick={onSignOut}>ログアウト</Btn>
          <Btn onClick={() => setModal({ type: 'add' })}>＋ 目標を追加</Btn>
        </div>
      </div>

      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        {/* Sidebar */}
        <div style={{ width: 210, flexShrink: 0, borderRight: '1px solid rgba(255,255,255,0.06)', padding: '18px 10px', background: 'rgba(255,255,255,0.01)', overflowY: 'auto' }}>
          <div style={{ fontSize: 10, color: '#404660', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 10, paddingLeft: 10 }}>組織階層</div>
