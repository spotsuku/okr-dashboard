...
export default function CsvPage({ levels, fiscalYear = '2026' }) {
  ...
  const handleRegister = async () => {
    ...
    for (const row of editRows) {
      ...
      // 年度プレフィックスを付与したperiodキーで保存
      const periodKey = toPeriodKey(row.period, fiscalYear)
      const { data: obj, error: e1 } = await supabase
        .from('objectives')
        .insert([{ title: row.title, owner: row.owner, level_id: level.id, period: periodKey }])
        .select().single()
      if (e1) { ng++; continue }
      if (row.krs?.length) {
      ...
      {step === 'preview' && aiSummary && (
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderRadius: 10, marginBottom: 16, background: fiscalYear === '2026' ? 'rgba(77,159,255,0.08)' : 'rgba(255,159,67,0.08)', border: `1px solid ${fiscalYear === '2026' ? 'rgba(77,159,255,0.25)' : 'rgba(255,159,67,0.25)'}` }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: fiscalYear === '2026' ? '#4d9fff' : '#ff9f43' }}>
              {fiscalYear}年度として登録されます
            </span>
            <span style={{ fontSize: 12, color: '#606880' }}>
              （periodは {fiscalYear === '2026' ? 'q1, q2...' : `${fiscalYear}_q1, ${fiscalYear}_q2...`} で保存）
            </span>
          </div>
      ...
      {step === 'done' && (
        <div style={{ textAlign: 'center', padding: '48px 20px' }}>
          ...
          <div style={{ fontSize: 14, color: '#606880', marginBottom: 28 }}>
            {fiscalYear}年度のOKRダッシュボードに反映されました
          </div>
