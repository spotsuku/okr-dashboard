...
export default function Dashboard({ user, onSignOut }) {
  ...
  // Load objectives whenever activeLevelId or activePeriod changes
  useEffect(() => {
    if (!activeLevelId) return
    const load = async () => {
      const { data, error } = await supabase
        .from('objectives')
        .select('id, level_id, period, title, owner, key_results!key_results_objective_id_fkey(id, objective_id, title, target, current_value, unit, lower_is_better)')
        .eq('level_id', activeLevelId)
        .eq('period', activePeriod)
        .order('id')
      if (error) console.error('objectives error:', error)
      else setObjectives(data || [])
    }
    load()
  }, [activeLevelId, activePeriod])
  ...
  const loadObjectives = useCallback(async (levelId, period) => {
    ...
    const { data, error } = await supabase
      ...
      .eq('period', period)
      .order('id')
    if (error) console.error('loadObjectives error:', error)
    else setObjectives(data || [])
  }, [])
  ...
  const handleSave = async ({ obj, krs }) => {
    ...
    if (krs.length && objectiveId) {
      await supabase.from('key_results').insert(
        krs.map(k => ({ title: k.title, target: k.target, current_value: k.current_value, unit: k.unit, lower_is_better: k.lower_is_better, objective_id: objectiveId }))
      )
    }
