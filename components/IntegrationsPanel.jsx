        ...
        const run = async () => {
                ...
                if (integResult === 'ok' && integService) {
                  ...
                  if (integServiceKey === 'google_gmail' || integServiceKey === 'google_calendar') {
                              try {
                                            // まず getSession() で現在のセッションを取得（OAuthコールバック直後はprovider_tokenが含まれる）
                                let session = null
                                            const { data: sessionData } = await supabase.auth.getSession()
                                            session = sessionData?.session

                                // provider_tokenがない場合はexchangeCodeForSession経由で取得を試みる
                                ...
                                if (session?.provider_token) {
                                                ...
                                                if (upsertError) {
                                                                  setErrorMsg(`トークン保存エラー: ${upsertError.message}`)
                                                }
                                } else {
                                                                                                                                        ...
                                                                                                                                        <div style={{ display: 'flex', gap: 6, marginTop: 'auto' }}>
                                                                                                                                          {!isConnected ? (
                                                                                                                                              <button
                                                                                                                                                                        ...
                                                                                                                                                                        style={{
                                                                                                                                                                                                    flex: 1, background: integ.brandColor, color: '#fff',
                                                                                                                                              ...
                                                                                                                                              <>
                                                                                                                                                {isExpired && (
                                                                                                                                                                          <button
                                                                                                                                                                                                        ...
                                                                                                                                                                                                        disabled={isBusy || !isViewingSelf}
                                                                                                                                                                                                        style={{
                                                                                                                                                                                                                                        flex: 1, background: T.warn, color: '#fff',
