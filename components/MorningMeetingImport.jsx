...
// ─── 朝会タスク取り込みモーダル ─────────────────────────────────────────
// 使い方: <MorningMeetingImport open={open} onClose={...} members={...} T={...} />
export default function MorningMeetingImport({ open, onClose, members = [], T }) {
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState(null)
  const [meetingInfo, setMeetingInfo] = useState(null) // { pageTitle, meetingDate, pageUrl }
  const [items, setItems] = useState([]) // [{ include, text, assignee, reportId, dueDate }]
  ...
  // モーダルを開いたら Notion 取得 + 候補KA読み込み
  useEffect(() => {
    if (!open) return
    setError(null); setSaved(null); setMeetingInfo(null); setItems([])
    setLoading(true)
    Promise.all([
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ flex:1, overflow:'auto', padding:16 }}>
          ...
          {!loading && !error && !saved && items.length === 0 && meetingInfo && (
            <div style={{ color: T?.textMuted || '#888', fontSize:13 }}>アクションアイテムが見つかりませんでした。Notionページに「アクションアイテム」見出しの配下にto-doブロックを入れてください。</div>
          )}
          {!loading && !error && !saved && items.length > 0 && (
            <>
              <div style={{ fontSize:11, color: T?.textMuted || '#888', marginBottom:8 }}>
        ...
        <div style={{ padding:'10px 16px', borderTop:`1px solid ${T?.borderMid || '#e5e5e5'}`, display:'flex', justifyContent:'flex-end', gap:8 }}>
          ...
          {!saved && (
            <button onClick={doImport} disabled={saving || loading || items.filter(x => x.include && x.text.trim()).length === 0}
              style={{ padding:'7px 14px', borderRadius:7, border:'none', background:'#4d9fff', color:'#fff', fontSize:12, cursor:'pointer', opacity: (saving || loading) ? 0.5 : 1, fontFamily:'inherit', fontWeight:600 }}>
